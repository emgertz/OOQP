...
newly-built libraries (see L<ENVIRONMENT VARIABLES AND
configure>). Note that newer versions of MA57 require the MeTiS
library.  If you are using a newer version of MA57, list both the MA57
library and the MeTiS library in the MA57LIB environment variable.
