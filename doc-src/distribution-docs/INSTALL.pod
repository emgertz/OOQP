...
We remark that the GNU C++ and FORTRAN (gfortran) compilers build OOQP
correctly.
...
machine-optimized library if one is available. Most Linux
distributions have a prepackaged version of BLAS that may be installed
using the distribution's package manager.  Information about
obtaining optimized versions of the BLAS for other architectures may
be found on the Netlib page http://www.netlib.org/blas/index.html
