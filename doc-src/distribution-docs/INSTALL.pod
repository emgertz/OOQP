...
For libraries, the value of the environment variable does not need to
be a file name, but may instead be the name of the library as you want
it to appear on the link line. For example, the setting
...
Once LIBBLAS and other variables of interest are set appropriately,
...
compiler obtain F2C (see the L<"USING F2C"> section below.)
...
http://www.cs.utk.edu/~ghenry/distrib/. Information about obtaining
...
Therefore, one option is to move an appropriate libblas.a file
manually to this location.
...
explicitly to point to it. See L<ENVIRONMENT VARIABLES AND configure>
...
MA27 is available free for noncommercial use. Detailed conditions of
use are given at the time of registration.
...
(we cannot offer such help to unregistered users.)

You will be mailed a user name and password that allows you to
...
appropriate for users running Linux on Intel hardware.
...
environment variable to point to this library. See L<ENVIRONMENT
VARIABLES AND configure> for information on how to do so.

The procedure for MA57 is similar, once you have obtained (and
licensed) a copy of the MA57 source from the HSL2000 collection. See 

  http://www.cse.clrc.ac.uk/Activity/HSL

for details on obtaining MA57 and other HSL2000 codes.
Once you have obtained MA27, unpack the files into 
...
OOQP page http://www.cs.wisc.edu/~swright/ooqp . Alternatively, you
...
You may install the library and library headers in a location other
...
environment variable. See L<ENVIRONMENT
VARIABLES AND configure> for information on how to do so.
...
page is http://www.mcs.anl.gov/tao/
...
=item  Linux (gcc version 2.95.3)

Set FFLAGS, CFLAGS and CXXFLAGS to C<-funroll-loops -O3 -mcpu=i586>

=item Solaris (SunPro compilers)

Set FFLAGS, CFLAGS and CXXFLAGS to C<-fast>
...
=head2 The configure script did not complete successfully.
...
script. If you suspect the latter, please contact the OOQP developers.

=head2 The program "make" terminates immediately without building OOQP.

If typing "make" results in

        make: Fatal error: No arguments to build

or some similar message, then the version of make that you are using
