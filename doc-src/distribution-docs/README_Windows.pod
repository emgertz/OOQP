...
The file L<INSTALL|distribution-docs::INSTALL> contains instructions for compiling OOQP
...
compiling under Windows. We are aware of two reasonable options. The
first is to install and use Cygwin. The second is to compile under
Microsoft Visual C++.
...
simply follow the standard instructions in the L<INSTALL|distribution-docs::INSTALL>
guide.
...
may work (see the INSTALL file for a discussion of F2C).  There are
also commercial packages that support for compiling Fortran with
